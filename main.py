...
from threads import neuronet_thread, RealThread2
...
if __name__ == "__main__":
    ...
    # queues = MetaQueue2.get_instance()

    # контейнер с командами на остановку нитей
    # kill_neuro = KillCommandsContainer.get_instance()
    kill = KillCommandsContainerN.get_instance()

    batch_size = 1
    # queues_m: MetaQueueN = MetaQueueN(batch_size)
    queues_m: MetaQueue2 = MetaQueue2.get_instance(kill)
    dispatcher: DispatcherAbstract = ListDispatcher(batch_size, kill)

    # Нить модели реального мира
    realWorldThread: Thread = RealThread2('realWorldThread', dispatcher, queues_m, InitialStatus(max_tests), kill.reality, batch_size)
